...
function buildFallbackCards(deckId: string, templateId: string, level: 'junior' | 'mid' | 'senior'): Flashcard[] {
  ...
  if (templateId === 'problem') {
    return [
      makeCard(deckId, 1, { type: 'problem', level, front: 'Calculate terminal value: CF = $10M, WACC = 10%, Growth = 2%.', back: 'Terminal Value = Expected CF ÷ (WACC − Growth Rate)\nTV = $10M ÷ (0.10 − 0.02)\nTV = $10M ÷ 0.08\nTV = $125 Million', difficulty: 4, templateKey: 'minimal_dark', colorPalette: 'cyan_blue', sourceContext: 'Corporate Valuation', tags: ['math', 'valuation'] }),
      makeCard(deckId, 2, { type: 'insight', level, front: 'Why is the Cost of Debt adjusted when calculating WACC?', back: 'Because interest payments are tax-deductible.\nAfter-Tax Cost of Debt = Rate × (1 − Tax Rate)\nThis reduces the effective cost of borrowing vs. equity.', difficulty: 3, templateKey: 'minimal_dark', colorPalette: 'rose_crimson', sourceContext: 'Cost of Capital', tags: ['formula', 'finance'] }),
      makeCard(deckId, 3, { type: 'example', level, front: 'How do you identify a production bottleneck step-by-step?', back: '1. Map all production stages\n2. Measure cycle time at each stage\n3. The stage with highest cycle time = bottleneck\n4. Ensure bottleneck always has upstream WIP\n5. Subordinate all other processes to it', difficulty: 3, templateKey: 'minimal_dark', colorPalette: 'emerald_teal', sourceContext: 'Operations Management', tags: ['framework', 'operations'] }),
      makeCard(deckId, 4, { type: 'concept', level, front: 'What is the Theory of Constraints (TOC)?', back: 'A management paradigm that every process has one bottleneck, and improving the bottleneck is the only way to increase throughput. All other improvements are waste unless the constraint is improved first.', difficulty: 3, templateKey: 'minimal_dark', colorPalette: 'indigo_violet', sourceContext: 'Operations Theory', tags: ['concept'] }),
    ];
  }

  // Default: Concept / Deep Learning
  return [
    makeCard(deckId, 1, { type: 'concept', level, front: 'What is Spaced Repetition (SRS)?', back: 'An evidence-based learning technique using increasing time intervals between reviews to exploit the psychological spacing effect — making memories exponentially stronger with each successful recall.', difficulty: 2, templateKey: 'minimal_dark', colorPalette: 'indigo_violet', sourceContext: 'Learning Theory', tags: ['memory', 'neuroscience'] }),
    makeCard(deckId, 2, { type: 'insight', level, front: 'Active Recall vs Passive Reading — what is the real difference?', back: 'Active Recall forces memory retrieval without looking, physically strengthening neural pathways.\nPassive Reading only stimulates recognition memory — creating an illusion of competence without real retention.', difficulty: 2, templateKey: 'minimal_dark', colorPalette: 'cyan_blue', sourceContext: 'Cognitive Psychology', tags: ['study-techniques'] }),
    makeCard(deckId, 3, { type: 'example', level, front: 'Give a concrete example of Interleaving in a study session.', back: 'Instead of 50 calculus → 50 algebra (blocked),\nmix: calculus, algebra, geometry, calculus, algebra… randomly.\nThis forces the brain to identify which concept applies — building flexible, transferable knowledge.', difficulty: 3, templateKey: 'minimal_dark', colorPalette: 'amber_orange', sourceContext: 'Skill Acquisition', tags: ['optimization'] }),
    makeCard(deckId, 4, { type: 'insight', level, front: 'Why does the Forgetting Curve flatten after multiple spaced reviews?', back: 'Each successful active recall thickens the myelin sheath around the neural circuit, increasing action potential speed and preventing synaptic pruning — making memory biologically permanent over time.', difficulty: 4, templateKey: 'minimal_dark', colorPalette: 'rose_crimson', sourceContext: 'Neurobiology', tags: ['biology', 'memory'] }),
    makeCard(deckId, 5, { type: 'concept', level, front: 'Define the SM-2 Algorithm.', back: 'The SuperMemo 2 spaced repetition algorithm that tracks ease factor (EF) and interval per card. Hard → shorter interval, Easy → longer. Cards rated Hard ≥ 3 consecutive times get reset to day 1 review.', difficulty: 3, templateKey: 'minimal_dark', colorPalette: 'cyan_blue', sourceContext: 'Spaced Repetition Systems', tags: ['algorithm'] }),
    makeCard(deckId, 6, { type: 'summary', level, front: 'Design a 1-week study plan using SRS principles.', back: '✓ Review new material same day it is learned\n✓ First recall: next day\n✓ Second recall: 3 days later\n✓ Third recall: 7 days later\n✓ Hard cards reset to Day 1 automatically', difficulty: 3, templateKey: 'minimal_dark', colorPalette: 'emerald_teal', sourceContext: 'Study Planning', tags: ['application', 'planning'] }),
  ];
}
